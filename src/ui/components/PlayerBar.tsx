...
const PlayerBar: React.FC<PlayerBarProps> = ({
  ...
  return (
    <motion.div 
      initial={{ y: 100 }}
      animate={{ y: 0 }}
      className="fixed bottom-0 left-0 right-0 h-24 md:h-28 bg-[var(--bg-glass)] backdrop-blur-3xl border-t border-white/5 z-[100] px-3 md:px-8 flex items-center transition-all duration-500 shadow-[0_-20px_50px_rgba(0,0,0,0.3)]"
    >
      {/* 1. SONG INFO (LEFT - 30% width on Desktop, flexible on Mobile) */}
      <div className="flex w-[40%] md:w-[30%] items-center gap-2 md:gap-4 shrink-0 min-w-0">
        <div className="relative group cursor-pointer shrink-0" onClick={() => window.dispatchEvent(new CustomEvent('toggleLyrics'))}>
          <img 
            ...
            className="w-10 h-10 md:w-14 md:h-14 rounded-xl object-cover border border-white/10 group-hover:border-emerald-500/50 transition-all shadow-lg"
            alt="Cover"
          />
        ...
        <div className="min-w-0 pr-2">
          <h4 className="text-[10px] md:text-sm lg:text-base font-black text-white truncate italic uppercase tracking-tight leading-tight">{currentSong.title}</h4>
          ...
          {sourceTitle && (
            <div className="hidden sm:flex items-center gap-1.5 mt-1 md:mt-1.5 bg-white/5 px-2 py-0.5 rounded-full w-fit max-w-full">
              <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse shrink-0" />
              <span className="text-[6px] md:text-[7px] font-black text-white/40 uppercase tracking-[0.15em] truncate">Suena de: {sourceTitle}</span>
            </div>
          )}
        </div>
      </div>

      {/* 2. PLAYER CONTROLS (CENTER - 40% width on Desktop, flexible on Mobile) */}
      <div className="flex-1 md:w-[40%] flex flex-col items-center gap-1 md:gap-3 px-1 md:px-4 relative z-10">
        <div className="flex items-center gap-2 sm:gap-6 md:gap-8 lg:gap-12">
          {/* Action Buttons - Compact Grouping for Mobile */}
          <div className="flex items-center gap-1 md:gap-3 mr-1 md:mr-0">
            <button 
              onClick={onToggleFavorite}
              className={`p-1.5 md:p-2 transition-all active:scale-75 ${currentSong.isFavorite ? 'text-emerald-500' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
            >
              <Heart className={`w-4 h-4 md:w-6 md:h-6 ${currentSong.isFavorite ? 'fill-current' : ''}`} />
            </button>
            <button 
              onClick={onAddToPlaylist}
              className="p-1.5 md:p-2 text-[var(--text-muted)] hover:text-[var(--text-main)] transition-all active:scale-75"
            >
          ...
          <div className="flex items-center gap-3 sm:gap-6 lg:gap-10 text-[var(--text-main)]">
            <button onClick={onPrev} className="p-1.5 md:p-2 text-[var(--text-muted)] hover:text-[var(--text-main)] transition-all active:scale-90">
              <SkipBack className="w-5 h-5 md:w-8 md:h-8 fill-current" />
            </button>
            <button 
              onClick={onTogglePlay}
              className="w-10 h-10 md:w-16 md:h-16 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full flex items-center justify-center transition-all active:scale-95 shadow-[0_10px_25px_rgba(16,185,129,0.4)] relative group"
            >
            ...
            <button onClick={onNext} className="p-1.5 md:p-2 text-[var(--text-muted)] hover:text-[var(--text-main)] transition-all active:scale-90">
              <SkipForward className="w-5 h-5 md:w-8 md:h-8 fill-current" />
            </button>
          </div>

          {/* EQ Toggle - Compact for Mobile */}
          <button 
            onClick={() => setIsEQOpen(!isEQOpen)}
            className={`p-1.5 md:p-2 transition-all active:scale-90 rounded-full ml-1 md:ml-0 ${isEQOpen ? 'text-emerald-500 bg-emerald-500/10' : 'text-[var(--text-muted)] hover:text-emerald-500'}`}
          >
            <SlidersHorizontal className="w-4 h-4 md:w-6 md:h-6" />
          </button>
        </div>
        
        {/* PROGRESS BAR - Refined for Mobile space */}
        <div className="w-full flex items-center gap-2 md:gap-4 relative max-w-xl">
          <span className="text-[8px] md:text-[9px] font-black text-[var(--text-muted)] w-8 md:w-10 text-right tabular-nums italic opacity-60 md:opacity-100">{formatTime(currentTime)}</span>
          
          <div className="relative flex-1 group h-0.5 md:h-1 bg-white/10 md:bg-[var(--border-color)] rounded-full cursor-pointer">
            <div className="absolute left-0 right-0 bottom-0 h-10 md:h-20 pointer-events-none z-0 overflow-hidden opacity-30">
            ...
            <input 
              ...
              className="absolute inset-x-0 -top-6 md:-top-8 bottom-0 w-full opacity-0 cursor-pointer z-30"
            />
            <div 
          ...
          <span className="text-[8px] md:text-[9px] font-black text-[var(--text-muted)] w-8 md:w-10 tabular-nums italic opacity-60 md:opacity-100">{formatTime(duration)}</span>
        </div>
      </div>

      {/* 3. EXTRA CONTROLS (RIGHT - 30% width on Desktop, Hidden on Mobile) */}
