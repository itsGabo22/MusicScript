...
import { Music, Heart, ListMusic, Plus, Library, Trash2, X } from 'lucide-react';
    ...
    <div className="w-72 h-full bg-[var(--bg-sidebar)] backdrop-blur-3xl border-r border-[var(--border-color)] flex flex-col p-6 overflow-hidden transition-colors duration-500">
      <div className="flex items-center justify-between mb-10 px-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-[var(--text-main)] uppercase tracking-tighter">MusicScript</h1>
        </div>
      ...
      <nav className="flex-1 space-y-8 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="space-y-1">
          <p className="px-4 text-[10px] font-black uppercase text-[var(--text-muted)] tracking-[0.2em] mb-3">Biblioteca</p>
          <NavItem 
        ...
        <div className="space-y-1">
          <div className="px-4 flex justify-between items-center mb-3">
            <p className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-[0.2em]">Playlists</p>
            <button 
      ...
      <div className="mt-6 pt-6 border-t border-[var(--border-color)]">
        <div className="bg-emerald-500/10 rounded-2xl p-4 border border-emerald-500/10">
          <p className="text-[10px] text-emerald-400 font-bold uppercase mb-1">Estructuras de Datos</p>
          <p className="text-xs text-[var(--text-muted)] font-medium">Proyecto Final - 2026</p>
        </div>
      </div>
    </div>
  );
...
const NavItem = ({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 font-bold text-sm ${
      active 
